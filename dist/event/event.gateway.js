...
let EventGateway = exports.EventGateway = class EventGateway {
    constructor() {
        this.onlineUsers = new Map();
    }
    addUser(client, payload) {
        const { userId } = payload;
        this.onlineUsers.set(Number(userId), client);
    }
    handleDisconnect(socket) {
        const userIdToRemove = Array.from(this.onlineUsers.entries()).find(([_, s]) => s === socket)?.[0];
...
], EventGateway.prototype, "addUser", null);
